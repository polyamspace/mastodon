...
import { loadCustomEmoji } from '@/flavours/polyam/features/emoji';
import { emojiLogger } from '@/flavours/polyam/features/emoji/utils';
...
export async function importCustomEmoji(emojis: ApiCustomEmojiJSON[]) {
  ...
  const { searchCustomEmojisByShortcodes, clearCache } =
    await import('@/flavours/polyam/features/emoji/database');
  ...
  if (existingEmojis.length > 0 && existingEmojis.length < emojis.length) {
    ...
    const { reloadCustomEmojis } =
      await import('@/flavours/polyam/features/emoji/picker');
