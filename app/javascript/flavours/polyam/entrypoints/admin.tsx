...
async function mountReactComponent(element: Element) {
  ...
  const { default: AdminComponent } =
    await import('@/flavours/polyam/containers/admin_component');
...
on('change', '#form_admin_settings_flavour_and_skin', ({ target }) => {
  if (target instanceof HTMLSelectElement) onChangeFlavourAndSkin(target);
});

// Polyam: User role preview

const updateRoleBadgePreview = (element: HTMLInputElement, text = false) => {
  ...
  if (preview) {
    for (const item of preview.querySelectorAll<HTMLDivElement>(
      ...
      if (text) {
        const e = item.querySelector('span');
        if (e) e.innerText = element.value;
      } else {
...
ready(() => {
  ...
  if (roleColorSelect) {
    updateRoleBadgePreview(roleColorSelect);
  }
