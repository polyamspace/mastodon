...
import MaximizeIcon from '@/awesome-icons/solid/maximize.svg?react';
import MinimizeIcon from '@/awesome-icons/solid/minimize.svg?react';
import PauseIcon from '@/awesome-icons/solid/pause.svg?react';
import PlayIcon from '@/awesome-icons/solid/play.svg?react';
import ExpandIcon from '@/awesome-icons/solid/up-right-and-down-left-from-center.svg?react';
...
class Video extends PureComponent {
  ...
  render () {
    const { preview, src, aspectRatio, inline, onOpenVideo, onCloseVideo, onOpenAltText, intl, alt, lang, letterbox, fullwidth, detailed, sensitive, editable, blurhash, autoFocus, matchedFilters } = this.props;
    ...
    // Polyam: Kept 16/9 ratio in timelines.
    return (
      <div style={{aspectRatio: inline ? '16 / 9' : aspectRatio}}>
        ...
        >
          ...
          {(revealed || editable) && <video
            ...
            style={{ aspectRatio: inline ? '16 / 9' : aspectRatio, width: '100%' }}
          />}
          ...
          <div className={classNames('video-player__controls', { active: paused || hovered })}>
            ...
            <div className='video-player__buttons-bar'>
              <div className='video-player__buttons left'>
                <button type='button' title={intl.formatMessage(paused ? messages.play : messages.pause)} aria-label={intl.formatMessage(paused ? messages.play : messages.pause)} className='player-button' onClick={this.togglePlay} autoFocus={autoFocus}><Icon id={paused ? 'play' : 'pause'} icon={paused ? PlayIcon : PauseIcon} /></button>
                <button type='button' title={intl.formatMessage(muted ? messages.unmute : messages.mute)} aria-label={intl.formatMessage(muted ? messages.unmute : messages.mute)} className='player-button' onClick={this.toggleMute}><Icon id={muted ? 'volume-off' : 'volume-up'} icon={muted ? MuteIcon : UnmuteIcon} /></button>
              ...
              <div className='video-player__buttons right'>
                ...
                {(!fullscreen && onOpenVideo) && <button type='button' title={intl.formatMessage(messages.expand)} aria-label={intl.formatMessage(messages.expand)} className='player-button' onClick={this.handleOpenVideo}><Icon id='expand' icon={ExpandIcon} /></button>}
                {onCloseVideo && <button type='button' title={intl.formatMessage(messages.close)} aria-label={intl.formatMessage(messages.close)} className='player-button' onClick={this.handleCloseVideo}><Icon id='compress' icon={CompressIcon} /></button>}
                <button type='button' title={intl.formatMessage(fullscreen ? messages.exit_fullscreen : messages.fullscreen)} aria-label={intl.formatMessage(fullscreen ? messages.exit_fullscreen : messages.fullscreen)} className='player-button' onClick={this.toggleFullscreen}><Icon id={fullscreen ? 'compress' : 'arrows-alt'} icon={fullscreen ? MinimizeIcon : MaximizeIcon} /></button>
              </div>
            </div>
          </div>
        </div>
