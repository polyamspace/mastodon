...
async function loadEmojisToStore() {
  const { store } = await import('@/flavours/polyam/store');
  const { loadCustomEmojis, loadLocale } =
    await import('@/flavours/polyam/reducers/slices/emojis');
...
async function handleEmojiDbReload() {
  ...
  const { store } = await import('@/flavours/polyam/store');
  const { needsReload } = await import('@/flavours/polyam/actions/app');
